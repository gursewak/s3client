...
class UploadBehavior extends ModelBehavior {
        ...
        foreach ($this->files as $field => $file) {
            ...
            if (!empty($file['old_filename'])) {
				...
				if(isset($this->settings[$Model->name][$field]['thumbnailimage']) && !empty($this->settings[$Model->name][$field]['thumbnailimage'])){
					$aws->deleteObject($this->settings[$Model->name][$field]['s3_bucket'], $setModelTableName.'/thumbnailimage/'.$file['old_filename']);
				}
				if(isset($this->settings[$Model->name][$field]['120x263']) && !empty($this->settings[$Model->name][$field]['120x263'])){
					$aws->deleteObject($this->settings[$Model->name][$field]['s3_bucket'], $setModelTableName.'/120x263/'.$file['old_filename']);
				}
				if(isset($this->settings[$Model->name][$field]['300x180']) && !empty($this->settings[$Model->name][$field]['300x180'])){
					$aws->deleteObject($this->settings[$Model->name][$field]['s3_bucket'], $setModelTableName.'/300x180/'.$file['old_filename']);
				}
                $aws->deleteObject($this->settings[$Model->name][$field]['s3_bucket'], $setModelTableName.'/original/'.$file['old_filename']);
            }
			...
			if(isset($this->settings[$Model->name][$field]['thumbnailimage']) && !empty($this->settings[$Model->name][$field]['thumbnailimage']) && (!empty($this->settings[$Model->name][$field]['thumbnailimage']['width']) && !empty($this->settings[$Model->name][$field]['thumbnailimage']['height']))){
				
				$width = $this->settings[$Model->name][$field]['thumbnailimage']['width'];
				$height = $this->settings[$Model->name][$field]['thumbnailimage']['height'];
					...
					$isUploaded = $aws->putObjectFile(
						  WWW_ROOT.'tmpImage/'.$file['name'],
						   $this->settings[$Model->name][$field]['s3_bucket'],
							$setModelTableName.'/thumbnailimage/'.$file['name'],
			...
			if(isset($this->settings[$Model->name][$field]['120x263']) && !empty($this->settings[$Model->name][$field]['120x263']) && (!empty($this->settings[$Model->name][$field]['120x263']['width']) && !empty($this->settings[$Model->name][$field]['120x263']['height']))){
				
				$width = $this->settings[$Model->name][$field]['120x263']['width'];
				$height = $this->settings[$Model->name][$field]['120x263']['height'];
				WideImage::load($file['tmp_name'])->resize($width, $height)->saveToFile('tmpImage/'.$file['name']);
						
					$isUploaded = $aws->putObjectFile(
						  WWW_ROOT.'tmpImage/'.$file['name'],
						   $this->settings[$Model->name][$field]['s3_bucket'],
							$setModelTableName.'/120x263/'.$file['name'],
						   $this->settings[$Model->name][$field]['s3_acl'],
						   $this->settings[$Model->name][$field]['s3_meta_headers'],
						   $this->settings[$Model->name][$field]['s3_request_headers']
					);
			...
			if(isset($this->settings[$Model->name][$field]['300x180']) && !empty($this->settings[$Model->name][$field]['300x180']) && (!empty($this->settings[$Model->name][$field]['300x180']['width']) && !empty($this->settings[$Model->name][$field]['300x180']['height']))){
				
				$width = $this->settings[$Model->name][$field]['300x180']['width'];
				$height = $this->settings[$Model->name][$field]['300x180']['height'];
				WideImage::load($file['tmp_name'])->resize($width, $height)->saveToFile('tmpImage/'.$file['name']);
						
					$isUploaded = $aws->putObjectFile(
						  WWW_ROOT.'tmpImage/'.$file['name'],
						   $this->settings[$Model->name][$field]['s3_bucket'],
							$setModelTableName.'/300x180/'.$file['name'],
						   $this->settings[$Model->name][$field]['s3_acl'],
						   $this->settings[$Model->name][$field]['s3_meta_headers'],
						   $this->settings[$Model->name][$field]['s3_request_headers']
					);
    ...
    function beforeDelete(Model $Model) {
        ...
        foreach ($this->settings[$Model->alias] as $field => $options) {
            ...
            if (!empty($filename)) {
				if(isset($this->settings[$Model->name][$field]['thumbnailimage']) && !empty($this->settings[$Model->name][$field]['thumbnailimage'])){
					$aws->deleteObject($options['s3_bucket'], $setModelTableName.'/thumbnailimage/'.$filename);
				}
				if(isset($this->settings[$Model->name][$field]['120x263']) && !empty($this->settings[$Model->name][$field]['120x263'])){
					$aws->deleteObject($options['s3_bucket'], $setModelTableName.'/120x263/'.$filename);
				}
				if(isset($this->settings[$Model->name][$field]['300x180']) && !empty($this->settings[$Model->name][$field]['300x180'])){
					$aws->deleteObject($options['s3_bucket'], $setModelTableName.'/300x180/'.$filename);
				}
                $aws->deleteObject($options['s3_bucket'], $setModelTableName.'/original/'.$filename);
            }
        }
	...
	function getUploadFolderInfo(Model $Model) {
		...
		foreach ($this->settings[$Model->alias] as $field => $options) {
			array_push($setTmpFolderInfo,'original');
			if(isset($this->settings[$Model->name][$field]['thumbnailimage']) && !empty($this->settings[$Model->name][$field]['thumbnailimage'])){
				array_push($setTmpFolderInfo,'thumbnailimage');
			}
			if(isset($this->settings[$Model->name][$field]['120x263']) && !empty($this->settings[$Model->name][$field]['120x263'])){
				array_push($setTmpFolderInfo,'120x263');
			}
			if(isset($this->settings[$Model->name][$field]['300x180']) && !empty($this->settings[$Model->name][$field]['300x180'])){
				array_push($setTmpFolderInfo,'300x180');
			}
		}
